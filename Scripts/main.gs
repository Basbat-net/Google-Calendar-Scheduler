...
function planSchedule() {
  ...
  // Check raw exam logs from database (column C), not the derived study array
  const rawExamLogs = (LOGGED_CALENDAR_EVENTS && LOGGED_CALENDAR_EVENTS["Examenes"]) ? LOGGED_CALENDAR_EVENTS["Examenes"] : [];
  const hasNoExams      = rawExamLogs.length === 0;

  // CASE 1: New fixed event(s) were added (I/J/K/L), new task logs (M), or no exams fetched
  // -> run full pipeline exactly as before
  if (hasNewFixedLogs || hasNewTaskLogs || hasNoExams) {
...
function clearAllCalendars(){
  ...
  clearAutomaticEvents_(fetchCalendar_("Proyectos"), now, end);
