...
function importWeek_(anyDate) {
  ...
  // *** MAPA DE OCUPACIÓN EN MEMORIA: occupancy[dayKey][col][row] = true ***
  // Necesario para detectar solapamientos ANTES de escribir en la hoja,
  // ya que leer celdas durante la importación siempre devolvería vacío.
  const occupancy = {};
  ...
  for (const cal of calendars) {
    ...
    for (const ev of events) {
      ...
      // Recopilar todos los bloques de 30 min que ocupa este evento
      const eventBlocks = [];
      let cur = new Date(s);
      while (cur < e) {
        ...
        if (baseCol && row) {
          eventBlocks.push({ dayKey, row, baseCol });
        }
      ...
      const { dayKey, baseCol } = eventBlocks[0];
      ...
      if (!laneByDayEventKey[laneKey]) {
        if (!occupancy[dayKey]) occupancy[dayKey] = {};

        // Comprobar si algún bloque del evento colisiona en la columna base
        const baseOcc   = occupancy[dayKey][baseCol] || {};
        const hasConflict = eventBlocks.some(({ row }) => !!baseOcc[row]);

        // Si hay conflicto, ir directamente a la columna de la derecha;
        // si no, quedarse en la base.
        let chosenCol = baseCol;
        if (hasConflict) {
          const rightCol = baseCol + 1;
          chosenCol = (rightCol <= b.rightLimit) ? rightCol : baseCol;
        }

        laneByDayEventKey[laneKey] = chosenCol;

        // Marcar ocupación en memoria
        if (!occupancy[dayKey][chosenCol]) occupancy[dayKey][chosenCol] = {};
        for (const { row } of eventBlocks) {
          occupancy[dayKey][chosenCol][row] = true;
        }
      }

      const destCol = laneByDayEventKey[laneKey];

      // Escribir bloques en la hoja + actualizar contadores
      for (const { row } of eventBlocks) {
        const bk = dayBounds[dayKeyFromDate_(new Date(s))]; // mismo day
        if (!bk || destCol > bk.rightLimit) continue;

        // CONTAR BLOQUES (30 min) de Estudio/Proyecto/Tareas
        if (calName === "Estudio" || calName === "Proyectos" || calName === "Tareas") {
...
function writeWeeklyCountsToMainDashboard_(ss, counters, rawOrderedSubjects) {
  ...
  shOut.getRange(CFG2.outProyectoA1).setValue(toHours(counters.byType30m["Proyectos"] || 0));
